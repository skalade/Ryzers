...
def save_anns(cv2_image, anns):
    ...
    plt.imshow(img)
    plt.axis('off')
    plt.savefig("/ryzers/sam_outputs/sam_test_output.jpg")
...
CHECKPOINT_PATH = "/ryzers/segment_anything/notebooks/sam_vit_h_4b8939.pth"

if not os.path.exists(CHECKPOINT_PATH):
    FILENAME = os.path.basename(urlparse(CHECKPOINT_URL).path)
    download_from_url(CHECKPOINT_URL, FILENAME)
    sam_checkpoint = FILENAME  
else:
...
desired_checkpoint_path = "/ryzers/notebooks/segment-anything/sam_vit_h_4b8939.pth"
if os.path.exists(sam_checkpoint) and os.path.abspath(sam_checkpoint) != os.path.abspath(desired_checkpoint_path):
    os.makedirs(os.path.dirname(desired_checkpoint_path), exist_ok=True)
    shutil.move(sam_checkpoint, desired_checkpoint_path)
